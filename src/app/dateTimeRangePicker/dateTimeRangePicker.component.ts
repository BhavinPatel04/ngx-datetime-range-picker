...
export class DateTimeRangePickerComponent implements OnChanges {
    ...
    @Output() public dateRangeModelChange: EventEmitter<
        Object
    > = new EventEmitter<Object>();
    @Output() public dateRangeChanged: EventEmitter<
        DateRangeModelItem
    > = new EventEmitter<DateRangeModelItem>();
    @Output() public inputFocusBlur: EventEmitter<Object> = new EventEmitter<
        Object
    >();
    @Output() public selectedDate: EventEmitter<Object> = new EventEmitter<
        Object
    >();
    ...
    public calendarAvailable: Object = {
    ...
    public selectedMinute: Object = {
        left: "" as string,
        right: "" as string,
    };
    public selectedMeridian: Object = {
        left: "" as string,
        right: "" as string,
    };
    ...
    public ngOnChanges(changes: SimpleChanges) {
        ...
        if (changes.dateRangeModel && !changes.dateRangeModel.firstChange) {
            const previousValue =
                changes.dateRangeModel.previousValue[this.config.selectedModel];
            const currentValue =
                changes.dateRangeModel.currentValue[this.config.selectedModel];
            if (
                previousValue &&
                currentValue &&
            ...
            changes.optionService.currentValue.subscribe(
                (dateOptions) => {
                    if (_.isObject(dateOptions) && !_.isArray(dateOptions)) {
                        this.options = dateOptions.plain
                            ? dateOptions.plain()
                            : dateOptions;
                    }
                },
                (err) => {
                    console.error(
                        "DateRangePickerComponent | Filter Call Failure: ",
                        err,
                    );
                },
                () => {
    ...
    public getDatecharacteristics(date, month, side) {
        ...
        const active: boolean = this.dateTimeRangePickerUtilityService.isDateActive(
            currentDate,
            startDate,
            endDate,
            side,
        );
        const today: boolean = this.dateTimeRangePickerUtilityService.isDateToday(
            currentDate,
            this.config,
        );
    ...
    public onCellClick(item, itemCell, side) {
        const date = item.date
            ? moment(item.date, DEFAULT_DATE_FROMAT).valueOf()
            : item.date;
    ...
    public onCellMouseEnter(item, itemCell): void {
        ...
        const endDate = this.config.endDate
            ? moment(this.config.endDate, DEFAULT_DATE_FROMAT).valueOf()
            : this.config.endDate;
        let hoverItemFirstDate = itemCell
            ? itemCell.getAttribute("firstDay")
            : "";
        let hoverItemLastDate = itemCell
            ? itemCell.getAttribute("lastDay")
            : "";
        const hoverItemText = itemCell ? itemCell.innerText : "";
        hoverItemFirstDate = moment(
            hoverItemFirstDate,
            DEFAULT_DATE_FROMAT,
        ).format(this.config.viewDateFormat);
        hoverItemLastDate = moment(
            hoverItemLastDate,
            DEFAULT_DATE_FROMAT,
        ).format(this.config.viewDateFormat);
    ...
    public updateCalendar() {
        ...
        if (!this.config.singleDatePicker) {
            ...
            this.dates.left = this.generateCalendar(
                this.config.startDate,
                "left",
            );
            if (this.config.timePicker) {
                this.times.left = this.generateTimePicker(
        ...
        if (this.config.timePicker) {
            this.times.right = this.generateTimePicker(
                this.config.endTime,
                "right",
            );
        }
    ...
    public onCalendarLabelChange(label, side, type) {
        ...
        if (this.config.type != "yearly") {
            const selectedMonth = `${this.selectedMonth[side]} ${
            ...
            if (
                this.selectedYear.left <= this.selectedYear.right &&
                side == "right"
            ) {
            ...
            const startDate = this.dateTimeRangePickerUtilityService.formatStartDate(
                config,
                this.config.viewDateFormat,
            );
            const endDate = this.config.endDate
                ? moment(this.config.endDate, DEFAULT_DATE_FROMAT).format(
                      this.config.viewDateFormat,
                  )
                : "";
    ...
    public onTimeLabelChange(timeItem, side, item) {
        ...
        if (side == "left") {
            ...
            const startDateEpoch = moment(
                this.config.startDate,
                DEFAULT_DATE_FROMAT,
            ).valueOf();
            const endDateEpoch = moment(
                this.config.endDate,
                DEFAULT_DATE_FROMAT,
            ).valueOf();
        ...
        if (timeItem == "hour") {
            this.selectedHour[
                side
            ] = this.dateTimeRangePickerUtilityService.convertToViewTimeItem(
                item,
            );
        } else {
            this.selectedMinute[
    ...
    public parseOptions(): void {
        ...
        this.config.selectedModel = this.config.selectedModel
    ...
    public parseOptionsToDefaultDateFormat() {
        ...
        if (this.config.timePicker) {
            this.config.minTime = this.config.minTime
                ? this.dateTimeRangePickerUtilityService.formatTimeToDefaultFormat(
                      this.config.minTime,
                  )
                : DEFAULT_START_TIME;
    ...
    public processDateRangeModel() {
        if (
            undefined != this.dateRangeModel &&
            !_.isEmpty(this.dateRangeModel)
        ) {
            if (this.dateRangeModel[this.config.selectedModel]) {
                const dateRangeMinDate = this.dateRangeModel[
                    this.config.selectedModel
                ].minDate;
                const dateRangeMaxDate = this.dateRangeModel[
                    this.config.selectedModel
                ].maxDate;
                const dateRangeStartDate = this.dateRangeModel[
                    this.config.selectedModel
                ].startDate;
                const dateRangeEndDate = this.dateRangeModel[
                    this.config.selectedModel
                ].endDate;
                const dateRangeDateArray = this.dateRangeModel[
                    this.config.selectedModel
                ].dateArray;

                this.config.dateArray = dateRangeDateArray
                    ? dateRangeDateArray
                    : this.config.dateArray;
                ...
                if (this.config.timePicker) {
                    const dateRangeMinTime = this.dateRangeModel[
                        this.config.selectedModel
                    ].minTime;
                    const dateRangeMaxTime = this.dateRangeModel[
                        this.config.selectedModel
                    ].maxTime;
                    const dateRangeStartTime = this.dateRangeModel[
                        this.config.selectedModel
                    ].startTime;
                    const dateRangeEndTime = this.dateRangeModel[
                        this.config.selectedModel
                    ].endTime;
                    this.config.minTime = this.config.minTime
                        ? this.dateTimeRangePickerUtilityService.formatTimeToDefaultFormat(
    ...
    public handleDateArray() {
        if (this.config.dateArray && this.config.dateArray.length > 0) {
            this.config.dateArray = this.dateTimeRangePickerUtilityService.getSanitizedDateArray(
                this.config,
            );
            ...
            const maxDate = this.config.dateArray[
                this.config.dateArray.length - 1
            ];
    ...
    public processRanges() {
        if (this.config.showRanges && !this.config.singleDatePicker) {
            if (_.isEmpty(this.config.availableRanges)) {
                this.config.availableRanges = this.dateTimeRangePickerUtilityService.createDefaultRanges(
                    this.config,
                );
            }
    ...
    public selectActiveRange() {
        _.forOwn(this.config.availableRanges, (rangeModel, range) => {
            if (
                this.config.startDate == rangeModel.startDate &&
                this.config.endDate == rangeModel.endDate
            ) {
    ...
    public generateCalendar(date, side) {
        this.selectedMonth[side] = moment(date, DEFAULT_DATE_FROMAT).format(
            "MMM",
        );
        this.selectedYear[
            side
        ] = this.dateTimeRangePickerUtilityService.getSelectedYear(
            this.config,
            date,
            side,
        );
        const calendarLabel = `${this.selectedMonth[side]} ${
            this.selectedYear[side]
        }`;
        ...
        const dates = {
            ...
            years: this.dateTimeRangePickerUtilityService.getYearsAvailable(
                this.config,
            ),
            itemRows: [] as Object[],
        };
        ...
        if (this.config.type != "yearly") {
            ...
            const monthStartWeekNumber: number = moment(
                date,
                DEFAULT_DATE_FROMAT,
            )
            ...
            let columns: any = 0;
            let currentItemDate = "";
            let datecharacteristics: Object = {};
            let rowItemText = "";
            ...
            if (this.config.type == "daily") {
                numberOfRows = this.dateTimeRangePickerUtilityService.getNumberOfWeeks(
                    date,
                );
                this.weekDayOptions = [
                    "su",
            ...
            for (let dateRows = 0; dateRows < numberOfRows; dateRows++) {
                ...
                rowVariables = this.dateTimeRangePickerUtilityService.getCalendarRowVariables(
                    rowOptions,
                );
                ...
                for (let rowItem = 0; rowItem <= columns; rowItem++) {
                    ...
                    rowItemOptions.year = this.selectedYear[side];
                    rowItemVariables = this.dateTimeRangePickerUtilityService.getCalendarRowItemVariables(
                        rowItemOptions,
                    );
                    ...
                    datecharacteristics = this.getDatecharacteristics(
                        currentItemDate,
                        calendarLabel,
                        side,
                    );
                    ...
                    if (
                        this.dateTimeRangePickerUtilityService.isRowIemValid(
                            rowOptions,
                        )
                    ) {
    ...
    public generateTimePicker(time, side) {
        ...
        for (let h = startHour; h <= 23; h++) {
            const stringified_h = this.dateTimeRangePickerUtilityService.convertToViewTimeItem(
                h,
            );
            timeObject.hour.push(stringified_h);
        }
        for (let m = startMinute; m <= 59; m++) {
            const stringified_m = this.dateTimeRangePickerUtilityService.convertToViewTimeItem(
                m,
            );
        ...
        if (side == "left") {
            this.config.startTime = `${this.selectedHour[side]}:${
                this.selectedMinute[side]
            }`;
        } else if (side == "right") {
            this.config.endTime = `${this.selectedHour[side]}:${
                this.selectedMinute[side]
            }`;
        }
    ...
    public updateActiveItemInputField() {
        ...
        if (!this.config.singleDatePicker) {
            ...
            itemFirstDate = moment(itemFirstDate, DEFAULT_DATE_FROMAT).format(
                this.config.viewDateFormat,
            );
            itemLastDate = moment(itemLastDate, DEFAULT_DATE_FROMAT).format(
                this.config.viewDateFormat,
            );
            if (this.config.type != "daily") {
    ...
    public getDateRangeModelItem(format?): DateRangeModelItem {
        ...
        let startDate = this.dateTimeRangePickerUtilityService.formatStartDate(
            this.config,
            outputDateFormat,
        );
        let endDate = moment(this.config.endDate, DEFAULT_DATE_FROMAT).format(
            outputDateFormat,
        );
    ...
    public doApply() {
        ...
        let outputStartDate = startDate
            ? moment(startDate, DEFAULT_DATE_FROMAT).valueOf()
            : null;
        let outputEndDate = endDate
            ? moment(endDate, DEFAULT_DATE_FROMAT).valueOf()
            : null;
        if (this.config.outputDateFormat) {
            outputStartDate = startDate
                ? moment(startDate, DEFAULT_DATE_FROMAT).format(
                      this.config.outputDateFormat,
                  )
                : null;
            outputEndDate = endDate
                ? moment(endDate, DEFAULT_DATE_FROMAT).format(
                      this.config.outputDateFormat,
                  )
                : null;
        }
    ...
    public doDateRangeModelChange() {
        const dateRangeModel = this.getDateRangeModel(
            this.config.inputDateFormat,
        );
    ...
    public updateActiveItem() {
        const startDate = this.dateTimeRangePickerUtilityService.getFirstLastDay(
            this.config.startDate,
            this.config.type,
        );
        const endDate = this.dateTimeRangePickerUtilityService.getFirstLastDay(
            this.config.endDate,
            this.config.type,
        );

        if (this.config.type == "weekly") {
            startDate.rowItemText = `W${this.dateTimeRangePickerUtilityService.getWeekNumber(
                startDate.firstDay,
            )}`;
            endDate.rowItemText = `W${this.dateTimeRangePickerUtilityService.getWeekNumber(
                endDate.firstDay,
            )}`;
        } else if (this.config.type == "monthly") {
            startDate.rowItemText = `${moment(
                startDate.firstDay,
                DEFAULT_DATE_FROMAT,
            ).format("MMM")}`;
            endDate.rowItemText = `${moment(
                endDate.firstDay,
                DEFAULT_DATE_FROMAT,
            ).format("MMM")}`;
        } else if (this.config.type == "quarterly") {
            ...
            endDate.rowItemText = `Quarter ${moment(
                endDate.firstDay,
                DEFAULT_DATE_FROMAT,
            ).quarter()}`;
        }
