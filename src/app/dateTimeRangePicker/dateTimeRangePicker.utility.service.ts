...
export class DateTimeRangePickerUtilityService {
    ...
    public formatDateToDefaultFormat(
        date,
        ...
        if (date) {
            if (!isNaN(Number(date))) {
                ...
                fromattedDate = moment(date, format).format(
                    DEFAULT_DATE_FROMAT,
                );
            }
        }
        ...
        if (time) {
            if (time.indexOf(":") > -1) {
                if (time.indexOf("AM") > -1 || time.indexOf("PM") > -1) {
                    formattedTime = moment(time, "h:mm A").format(
                        DEFAULT_TIME_FORMAT,
                    );
                } else {
                    formattedTime = time;
                ...
                console.warn(
                    "M1 Date Range Picker: The provided time is not in correct format. Format: HH:mm or hh:mm A",
                );
            }
        }
    ...
    public createDefaultRanges(config): Object {
        ...
        const formattedMaxDate = this.formatDateToDefaultFormat(
            maxDate,
            DEFAULT_DATE_FROMAT,
        );
    ...
    public getYearlyWeekCount(year, retailCalendar?) {
        ...
        if (year) {
            ...
            const yearStartWeekEndDate = moment(
                yearStartDate,
                DEFAULT_DATE_FROMAT,
            )
                .endOf("week")
    ...
    public getMonthsAvailable(minDate, maxDate, selectedYear) {
        ...
        if (minDate && maxDate && selectedYear) {
            ...
            const diff: number = moment(maxDatems).diff(
                moment(minDatems),
                "months",
            );
            const maxMonths =
                diff < MONTHS_AVAILABLE.length ? diff : MONTHS_AVAILABLE.length;

            for (let i = 0; i <= maxMonths; i++) {
                if (minDateMonthNumber >= MONTHS_AVAILABLE.length) {
                    months.push(
                        MONTHS_AVAILABLE[
                            minDateMonthNumber - MONTHS_AVAILABLE.length
                        ],
                    );
                } else {
    ...
    public getYearsAvailable(config) {
        ...
        if (minDate && maxDate) {
            const minYear = Number(
                this.getSelectedYear(config, minDate, "left"),
            );
            const maxYear = Number(
                this.getSelectedYear(config, maxDate, "right"),
            );
    ...
    public isDateActive(date, startDate, endDate, side): boolean {
        let active = false;
        if (
            (date == startDate && side == "left") ||
            (date == endDate && side == "right")
        ) {
            active = true;
        }
        return active;
    ...
    public isDateToday(date, config): boolean {
        ...
        const todayDate = moment()
            ...
            .valueOf();
        const type = config.type;
        const firstLastDay = this.getFirstLastDay(
            moment(todayDate).format(DEFAULT_DATE_FROMAT),
            type,
        );
        const firstDay = moment(
            firstLastDay.firstDay,
            DEFAULT_DATE_FROMAT,
        ).valueOf();
        const lastDay = moment(
            firstLastDay.lastDay,
            DEFAULT_DATE_FROMAT,
        ).valueOf();
    ...
    public formatStartDate(config, returnFormat) {
        ...
        if (startDate) {
            formattedStartDate = moment(startDate, DEFAULT_DATE_FROMAT).format(
                returnFormat,
            );
            if (type == "weekly") {
    ...
    public getWeekNumber(date): any {
        if (date) {
            ...
            return Math.ceil(
                ((datems.getTime() - yearStartms.getTime()) / 86400000 +
                    yearStartms.getDay() +
                    1) /
                    7,
            );
